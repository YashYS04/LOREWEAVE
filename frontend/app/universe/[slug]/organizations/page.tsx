      ...
      <Link href={`/universe/${slug}/organizations/${org.id}`} className="block space-y-2">
        ...
        {org.leader && (
          <p className="text-xs text-muted-foreground">
            <span className="font-medium">Leader:</span> {org.leader}
          </p>
        )}
        {org.description && (
  ...
  return (
    ...
    >
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <EntityHeader
          title="Organizations"
          subtitle={`${data?.total ?? 0} organization${(data?.total ?? 0) !== 1 ? "s" : ""}`}
          action={
            <Button size="sm" onClick={() => router.push(`/universe/${slug}/organizations/new`)}>
      ...
      {isLoading ? (
        ...
        <EntityEmptyState
          ...
          description="Define the factions, guilds, and societies of your world."
          actionLabel="New Organization"
          onAction={() => router.push(`/universe/${slug}/organizations/new`)}
        />
      ) : filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((o, i) => (
            <OrgCard key={o.id} org={o} slug={slug} index={i} />
          ))}
        </div>
      )}
    </EntityPageShell>
