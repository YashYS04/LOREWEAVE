...
export default function OrganizationProfilePage({ params }: PageProps) {
  ...
  const { data: item, isLoading, isError } = useOrganization(id);
  const update = useUpdateOrganization(id, uid);
  const del = useDeleteOrganization(uid);
  const { data: relData } = useEntityRelationships(uid, id, "organization");
  ...
  const onDelete = async () => {
    ...
    router.push(`/universe/${slug}/organizations`);
  };
  ...
  if (isError || !item) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 text-center">
        ...
        <Button asChild>
          <Link href={`/universe/${slug}/organizations`}>Back</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <EntityPageShell
      breadcrumbs={[
        { label: "Organizations", href: `/universe/${slug}/organizations` },
        { label: item.name },
      ]}
    >
          ...
          >
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-2">
                ...
                {item.leader && (
                  <p className="text-sm text-muted-foreground">Led by {item.leader}</p>
                )}
              </div>
              ...
              {activeTab === "overview" ? (
                <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="divide-y divide-border">
                    ...
                    {item.purpose && (
                      <div className="py-6">
                        <Section title="Purpose" content={item.purpose} />
                      </div>
                    )}
                    {item.notes && (
                      <div className="py-6">
            ...
            <form onSubmit={handleSubmit(onSave)} className="space-y-10">
              <EntityFormSection title="Basic Information">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Leader">
                    <input className={inputCls} {...register("leader")} />
                  </Field>
                </div>
              </EntityFormSection>
              ...
              <EntityFormSection title="Purpose">
                <Field label="Purpose">
                  <textarea className={inputCls} rows={3} {...register("purpose")} />
                </Field>
              </EntityFormSection>
