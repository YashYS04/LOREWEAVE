...
import { useWorldRule, useUpdateWorldRule, useDeleteWorldRule } from "@/hooks/use-world-rules";
...
export default function WorldRuleProfilePage({ params }: PageProps) {
  ...
  const { data: item, isLoading, isError } = useWorldRule(id);
  const update = useUpdateWorldRule(id, uid);
  const del = useDeleteWorldRule(uid);
  const { data: relData } = useEntityRelationships(uid, id, "world_rule");
  ...
  const onDelete = async () => {
    ...
    router.push(`/universe/${slug}/rules`);
  };
  ...
  if (isError || !item) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 text-center">
        ...
        <h1 className="text-2xl font-bold">World rule not found</h1>
        <Button asChild>
          <Link href={`/universe/${slug}/rules`}>Back</Link>
        </Button>
      </div>
    );
  }
        ...
        {!isEditing ? (
                ...
                >
                  <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-4">
                    <p className="mb-3 text-sm font-medium text-destructive">
                      Delete &ldquo;{item.title}&rdquo;? This action cannot be undone.
                    </p>
                    <div className="flex gap-2">
                      <Button
                        variant="destructive"
              ...
              {activeTab === "overview" ? (
                <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="divide-y divide-border">
                    ...
                    {(item.limitations || item.exceptions) && (
                      <div className="grid gap-6 py-6 sm:grid-cols-2">
                        <Section title="Limitations" content={item.limitations} />
                        <Section title="Exceptions" content={item.exceptions} />
                      </div>
                    )}
                    {item.notes && (
                      <div className="py-6">
          ...
          >
            <div className="flex items-center justify-between">
              <h1 className="text-2xl font-bold">Edit World Rule</h1>
              ...
              <EntityFormSection title="Limitations & Exceptions">
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="Limitations">
                    <textarea className={inputCls} rows={3} {...register("limitations")} />
                  </Field>
                  <Field label="Exceptions">
                    <textarea className={inputCls} rows={3} {...register("exceptions")} />
                  </Field>
                </div>
              </EntityFormSection>
