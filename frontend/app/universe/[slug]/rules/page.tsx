      ...
      <Link href={`/universe/${slug}/rules/${rule.id}`} className="block space-y-2">
        ...
        {rule.limitations && (
          <p className="text-xs text-muted-foreground">
            <span className="font-medium">Limitations:</span> {rule.limitations}
          </p>
        )}
  ...
  return (
    ...
    >
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <EntityHeader
          title="World Rules"
          subtitle={`${data?.total ?? 0} rule${(data?.total ?? 0) !== 1 ? "s" : ""}`}
          action={
            <Button size="sm" onClick={() => router.push(`/universe/${slug}/rules/new`)}>
      ...
      {isLoading ? (
        ...
        <EntityEmptyState
          icon={BookOpen}
          title="No rules yet"
          description="Define the magic systems, laws, and physics of your universe."
          actionLabel="New Rule"
          onAction={() => router.push(`/universe/${slug}/rules/new`)}
        />
      ) : filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((r, i) => (
            <RuleCard key={r.id} rule={r} slug={slug} index={i} />
          ))}
        </div>
      )}
    </EntityPageShell>
