...
export default function NewObjectPage({ params }: PageProps) {
  ...
  return (
    <EntityPageShell
      breadcrumbs={[
        { label: "Objects", href: `/universe/${slug}/objects` },
        { label: "New Object" },
      ]}
    >
      <motion.div
      ...
      >
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight">New Object</h1>
          <p className="mt-1 text-muted-foreground">Add an artifact, weapon, or relic.</p>
        </div>
        {create.isError && (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                <Field label="Name" required error={errors.name?.message}>
                  <input className={inputCls} placeholder="Object name" {...register("name")} />
                </Field>
              </div>
              ...
              <Field label="Owner">
                <input
                  className={inputCls}
                  placeholder="Current owner or holder"
                  {...register("owner")}
                />
              </Field>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Abilities">
                <textarea
                  className={inputCls}
                  rows={3}
                  placeholder="Special powers or properties..."
                  {...register("abilities")}
                />
              </Field>
            </div>
          ...
          <div className="flex gap-3 border-t border-border pt-6">
            ...
            <Button
              type="button"
              variant="ghost"
              onClick={() => router.push(`/universe/${slug}/objects`)}
            >
              Cancel
            </Button>
          </div>
